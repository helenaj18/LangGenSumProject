...
for counter in range(10):
    ...
    for filename in files:
        ...
        new_summary_file_name = filename[:-4].replace('/gendered/female_only' + str(counter) + '/', "/gendered_summaries_legal_led/female_only" + str(counter) + "/")+'.txt'
...
for counter in range(10):
    ...
    for filename in files:
        ...
        new_summary_file_name = filename[:-4].replace('/gendered/male_only' + str(counter) + '/', "/gendered_summaries_legal_led/male_only" + str(counter) + "/")+'.txt'
        new_dir = new_summary_file_name.split("A")[0]
        # Check whether the specified path exists or not
        if not os.path.exists(new_dir):
            # Create a new directory because it does not exist
